...
- R23. Extending R18, the "Request a feature" page is an interview: an assistant asks the PM one question at a time, drawn from the same readiness rubric as H7 and H10, until the request is ready; the five request fields (R18) fill in live beside the chat with a readiness chip. The interview stops when the silent score reaches clarity 4 or higher with a stated scope and at least three checkable acceptance criteria, or at eight questions, whichever comes first. The PM reviews the prefilled form (still R18's endpoint, unchanged) and clicks "Review and file" to submit. The filed GitHub issue gains a collapsed "How this request was refined (assistant interview)" section carrying the PM's self-score against the rubric and the interview transcript, so engineering's triage (H7) sees how the request got ready. Escape hatch: "Skip the interview, fill the form" shows the plain R18 form at any time, so the interview never blocks filing a request. The assistant reads the product maps and UI conventions before asking, so it does not ask what the product already answers. Every question carries a recommended answer, shown first among the options. The PM can finish with what we have at any point, ending the interview with the draft as it stands.
- R24. Bug intake: a second GitHub issue form, `bug-report.yml` (What happened, What you expected, Steps to reproduce, Where — staging or production, and the browser — and Your role), labeled `bug` and reachable from the same issue chooser as R18's request form. Bug reports are never scored with the readiness rubric: triage skips them, and `/implement-issue <n>` routes a `bug` issue to systematic debugging (reproduce, root cause, the failing test that captures it, the fix, the proof) instead of the interview, spec and plan, ending in the same pull requests. `npm run check:issue-form` validates both forms.
...
- F2. Part 1 timing target inside 75 minutes: 8 minutes framing and app tour, 14 minutes the room files requests, 8 minutes triage on screen ending with the facilitator choosing the issue, 30 minutes implementation on screen (the interview, the design, the plan, then TDD), 10 minutes merge, staging, smoke, promote, production, and 5 minutes of recovery buffer. The buffer is not a fallback: there is no pre-built PR and no recording. It is slack for a slow CI run or a provider retry. Cutoff rule: at minute 70 the facilitator names the live steps still running and stops driving them from the front; minutes 70 to 75 are buffer and questions, Part 2 starts at minute 75, and whatever finishes later is shown at the start of Part 2.
