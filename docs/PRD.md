...
- R15. Failures are visible, not silent. A failed breakdown shows a retry control. Skipped means the task was too short to break down, and the UI says so.
- R16. Rate limit: 20 breakdowns per user per hour, enforced with Redis, returned as a proper rate-limited error. The limit is configurable by environment variable.
...
- R19. `GET /health` returns status, git commit SHA, and environment name. The smoke test uses the SHA to know the new build is live.
...
- P4. After a staging deploy, a smoke workflow in the web repo polls `GET /health` on staging until the commit SHA matches the merged commit, then runs the Playwright smoke test against the staging URL. Its result is a required check on PRs from `develop` to `main`.
...
- F2. Part 1 timing target: 10 minutes framing and app tour, 15 minutes the room files requests, 10 minutes triage on screen, 25 minutes implementation on screen, 15 minutes merge, staging, smoke, promote, production.
...
- Any production-grade cost controls beyond the per-user rate limit.
...
- O5. Anthropic API budget for the session: 20 attendees creating tasks freely. Estimate cost per breakdown and set a session-wide cap if needed.
...
- Smoke test polls the health endpoint for the new commit SHA.
