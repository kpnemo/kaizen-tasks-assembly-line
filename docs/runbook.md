...
Only the app pull requests carry `Closes kpnemo/kaizen-tasks-assembly-line#<n>`; the docs one is merged for tidiness and closes nothing. If the issue is still `OPEN` after the merge, run `gh issue close <n> --repo kpnemo/kaizen-tasks-assembly-line --comment "Shipped in <pr url>"` and note it in section 8 so the implement skill gains that step. When the API changed too, merge and promote the API pull request first, then the web one.
