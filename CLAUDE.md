...
- Pipeline control room: DONE on staging (2026-09-11 12:53Z): API (kaizen-tasks-api #21), page (kaizen-tasks-web #31), `ship.yml` proven on 1.4.0. Production is 1.4.0 without them. **Harness issue #28 tracks today's work at stage `staging`**: pressing "Deploy 1.5.0 to production" on its row (or `gh workflow run ship.yml -f issues=28 -f version=1.5.0 ...`) ships everything on develop as 1.5.0 and closes it. Facilitator email `kpnemo@gmail.com`; the five variables are on the api service in both environments. Mike runs the end-to-end himself in a fresh session (new process picks up the edited skills). Handover page: the "Kaizen Assembly Line Handover" artifact (2026-09-11 evening).
