...
- Live: **release 1.5.0 in production and staging (2026-09-11 18:56Z)**: the pipeline control room (API kaizen-tasks-api #21, page kaizen-tasks-web #31, `ship.yml`), the shadcn rework, the footer fix. `/pipeline` serves in both environments with the facilitator buttons for `kpnemo@gmail.com`; #19, #22 and #28 shipped; `staging-label` runs on Railway's `deployment_status`; `ship.yml` resumes on rerun.
- Pipeline control room: DONE in production. Next: Mike's own end-to-end rehearsal in a fresh session (file → /triage-requests → /implement-issue → Deploy to staging → Deploy to production, all from the page); the next release is 1.6.0. Give every batch of work a tracking issue up front: the page only shows rows for issues. Handover page: the "Kaizen Assembly Line Handover" artifact (2026-09-11 evening).
