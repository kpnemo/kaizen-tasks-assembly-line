...
- Pipeline control room: DONE on staging (2026-09-11 12:53Z): API (kaizen-tasks-api #21), page (kaizen-tasks-web #31), `ship.yml` proven on 1.4.0. Production is 1.4.0 without them; the first "Deploy to production" from the page ships 1.5.0 with everything on develop. Facilitator email `kpnemo@gmail.com`; the five variables are on the api service in both environments. Next: Mike's own end-to-end rehearsal (file → /triage-requests → /implement-issue → Deploy to staging → Deploy to production), then fix what it finds; runbook and playbook updated for the page (docs/pipeline-page-runbook).
